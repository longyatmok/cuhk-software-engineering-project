...
var DemoOneRegion = function(opts) {
	...
	for ( var i = 0, l = geometry.faces.length; i < l; i++) {
		...
		face.vertexColors[2] = new THREE.Color().setHSL(
				srand.random() * 0.2 + 0.5, 0.75, srand.random() * 0.25 + 0.75);
		face.vertexColors[3] = new THREE.Color().setHSL(
				srand.random() * 0.2 + 0.5, 0.75, srand.random() * 0.25 + 0.75);
	...
	for ( var i = -1; i < 100; i++) {
		...
		for ( var j = 0; j < 19; j++) {
			...
			mesh.position.y = Math.floor(srand.random() * 12) * 20 + 160;
			mesh.position.z = Math.floor(srand.random() * 12) * 20 - 80;
			material.color.setHSL(srand.random() * 0.2 + 0.5, 0.75, srand
					.random() * 0.25 + 0.75);
