...
class Stat{
	...
	public static function getSelfHighest($needTop10){
	...
	public static function getSelfFastest(){
		$sql1 = (!!$needTop10) ? ', `'.self::TABLE.'` as `b`' : '';
	...
	public static function getGlobalHeight(){ // show top ten in the world
		...
		$pS->execute();
		$result = $pS->fetchAll();
		if($row = $this->getSelfHighest() !== false){
			$result[] = $row;
		}
		return $result;
	}
	public static function getGlobalTime(){ // show top ten in the world
		...
		$pS->execute();
		$result = $pS->fetchAll();
		if($row = $this->getSelfFastest() !== false){
			$result[] = $row;
		}
		return $result;
	}
	...
	public static function getSceneSelfHighest($needTop10){
		$result = array();
		$sql1 = (!!$needTop10) ? ', `'.self::TABLE.'` as `b`' : '';
		$sql2 = (!!$needTop10) ? ' AND `b`.`height` < `a`.`height` HAVING COUNT(`b`.`height`)>10' : '';
	...
	public static function getSceneSelfFastest(){
		$result = array();
		$sql1 = (!!$needTop10) ? ', `'.self::TABLE.'` as `b`' : '';
		$sql2 = (!!$needTop10) ? ' AND `b`.`time` < `a`.`time` HAVING COUNT(`b`.`time`)>10' : '';
	...
	public static function getSceneGlobalHeight(){ // show top ten in the world
		...
		$pS = $GLOBALS['PDO']->prepare($sql);
		$pS->execute();
		$sceneR = $this->getSceneSelfHighest();
		foreach(Scene::getAllID() as $sceneID){
